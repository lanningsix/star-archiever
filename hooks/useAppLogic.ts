/*
 * :file description:
 * :name: \star-archiever\hooks\useAppLogic.ts
 * :author: liuwei
 * :copyright: (c) 2025, Tungee
 * :date created: 2025-11-20 18:11:53
 * :last editor: liuwei
 * :date last edited: 2025-11-20 18:35:36
 */
...
export const useAppLogic = () => {
  ...
  const [themeKey, setThemeKey] = useState<ThemeKey>(
    () => (localStorage.getItem('app_theme') as ThemeKey) || 'lemon'
  )
  const [familyId, setFamilyId] = useState(
    () => localStorage.getItem('app_family_id') || ''
  )
  ...
  useEffect(
    () => localStorage.setItem('app_rewards', JSON.stringify(rewards)),
    [rewards]
  )
  useEffect(
  ...
  useEffect(
    () => localStorage.setItem('app_balance', balance.toString()),
    [balance]
  )
  useEffect(
    () =>
  ...
  useEffect(() => {
    if (!familyId || !isSyncReady) return
    const t = setTimeout(
      () => syncData('activity', { logs, balance, transactions }, true),
      500
    )
    return () => clearTimeout(t)
  ...
  useEffect(() => {
    if (!familyId || !isSyncReady) return
    const t = setTimeout(
      () => syncData('settings', { userName, themeKey }, true),
      1500
    )
    return () => clearTimeout(t)
  ...
  const updateBalance = (
    ...
    const newTx: Transaction = {
      ...
      type:
        amount > 0
          ? 'EARN'
          : amount < 0 && description.includes('兑换')
          ? 'SPEND'
          : 'PENALTY',
    }
  ...
  const redeemReward = (reward: Reward) => {
    if (balance >= reward.cost) {
      if (
        window.confirm(
          `确定要花费 ${reward.cost} 星星兑换 "${reward.title}" 吗？`
        )
      ) {
  ...
  const manualSaveAll = async (fid = familyId) => {
    ...
    try {
      ...
      await cloudService.saveData(fid, 'activity', {
        logs,
        balance,
        transactions,
      })
